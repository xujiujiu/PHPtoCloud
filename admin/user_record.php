...
<div class="con" style="background:#EBF2F9;">
		<div class="top"><p>&nbsp;&nbsp;&nbsp;<i class="fa fa-tint"></i>&nbsp;<b>管理员操作记录表</b>&nbsp;>>&nbsp;查看管理员操作信息</p></div>    
</div>
<div class="con">
   <table  width="100%" align="center" cellpadding="0" cellspacing="0"  >
  	<tr class="biaoti" bgcolor="#3F89EC">
    	<td class="first" nowrap  align="center" ><font color="#FFFFFF"><strong>日志编号</strong></font></td>
      	<td nowrap  align="center"><font color="#FFFFFF"><strong>管理员编号</strong></font></td>
 ...
 <?php 
	 ...
	 $pagesize = 20;//页面尺寸
	 //记录数据总数
	 $result2=mysql_query("select count(*) from user_record");
	 ...
	 if($page>$pages || $page == 0){
	   ...
	   echo  "location.href='user_record.php';"; 
	   echo "</script>";
       exit;
	 }
	 $offset=($page-1)*$pagesize;         //获取limit的第一个参数的值
	 $result3=mysql_query("select * from user_record limit $offset,$pagesize");
	 //var_dump($result3);die;
	 $user=mysql_fetch_assoc($result3);
	 //print_r($rem);die;
	 if($user){
		 do{
			$result4=mysql_query("select ID from user where user_name = '$user[user_name]'");
	 		$user_id = mysql_fetch_array($result4);
	 ?>
     <tr>
       	<td class='first'><?php echo $user['id']?></td>
        <td><?php echo $user_id['ID']?></td>
      	<td><?php echo $user['user_operation']?></td>
      	<td><?php echo $user['user_opdate']?></td>
    ...
    <form class="page" method="get" action="">
  		<div align="center">
			...
			//判断首页与尾页
				...
				$last=$pages;
				if ($page > 1){
					echo "<a href='user_record.php?page=".$first."'>&nbsp;&nbsp;首页&nbsp;&nbsp;</a> ";
					echo "<a href='user_record.php?page=".$prev."'>上一页&nbsp;&nbsp;</a>";	
				}

				if ($page < $pages){
					echo "<a href='user_record.php?page=".$next."'>&nbsp;&nbsp;下一页</a>"; 
					echo "<a href='user_record.php?page=".$last."'>&nbsp;&nbsp;尾页</a> ";
				}
